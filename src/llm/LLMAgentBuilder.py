...
class LLMAgentBuilder:
    ...
    def get_agent_llm(self) -> BaseLanguageModel:
        """获取Agent使用的语言模型"""
        return self.llm_chain_builder.init_chat_model(self.config)
    ...
    def create_multimodal_agent_executor(self, verbose: bool = True,
                                         callback_manager: CallbackManager = None) -> AgentExecutor:
        """创建多模态Agent执行器"""
        llm = self.get_agent_llm()
